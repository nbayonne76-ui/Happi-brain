...
### 12.3 Palette de couleurs H'appi
```css
--happi-primary:   #7C3AED  /* violet principal */
--happi-secondary: #06B6D4  /* cyan accent */
--happi-dark:      #0F0A1E  /* fond sombre */
--happi-card:      #1A1035  /* carte dark */
--happi-border:    rgba(255,255,255,0.08)
--happi-gradient:  linear-gradient(135deg, #7C3AED 0%, #06B6D4 100%)
```

### 12.4 Refonte site H'appi — ce qui a été changé (avril 2026)

**Problème** : Site vitrine générique, peu premium, pas de différenciation visuelle.

**Solution appliquée** :
1. **Page Home** : Hero avec AnimatedMesh (orbes violet/cyan), counter animé (+850 clients, etc.), TiltCard sur les use-cases, section "Nos Réalisations" avec filtres secteur
2. **Page Atelier** : Processus en étapes avec timeline animée, tech stack avec logos, pricing cards avec hover effects
3. **Page Cas d'usage** : Grille filtrée par secteur (meuble, hôtellerie, agroalim...), cards avec metrics clés
4. **Navigation** : Sticky header avec blur, MagneticButton pour le CTA principal
5. **Footer** : Liens structurés, newsletter, réseaux sociaux

**Règles de qualité visuelle** :
- Toujours utiliser `backdrop-filter: blur(12px)` sur les cards glass
- Dégradés : toujours de violet vers cyan (jamais l'inverse)
- Animations : max 0.6s duration, `ease: [0.22, 1, 0.36, 1]` pour les slides
- Mobile : tester à 375px et 390px (iPhone SE et iPhone 15)
...
### 13.1 Nouveautés IA (mai-juin 2026)

#### Claude (Anthropic)
- **Claude 4 Opus** : Sorti avril 2026. 200K tokens context window. Meilleur en raisonnement multi-étapes et coding complexe. Pricing : $15/$75 per MTok (input/output)
- **Claude 4 Sonnet** : Rapport coût/performance optimal pour chatbots. $3/$15 per MTok. ← **Recommandé pour tous nos bots**
- **Claude 4 Haiku** : Ultra-rapide pour les cas simples. $0.25/$1.25 per MTok
- **Extended Thinking** : Disponible sur Opus et Sonnet 4. Activer avec `thinking: { type: 'enabled', budget_tokens: 10000 }`. Très utile pour l'analyse post-appel complexe.
- **Computer Use** : Stable en prod. Claude peut contrôler un navigateur headless. Use case H'appi : automatiser les dashboards clients.

#### OpenAI
- **GPT-5** : Sorti mai 2026. Multimodal natif (text + image + audio). Context : 256K tokens. Moins bon que Claude 4 sur le français naturel selon nos tests.
- **o3** : Modèle de raisonnement. Excellent pour les maths et code. Trop lent pour les chatbots temps-réel (>30s parfois).
- **Realtime API** : Stable. Concurrent direct de Vapi.ai pour la voix. Latence ~300ms. Pricing plus élevé que Vapi.

#### Google
- **Gemini 2.5 Pro** : 1M tokens context. Fort sur les documents longs. Moins bon sur le dialogue naturel. Pricing compétitif.
- **Gemini 2.5 Flash** : Très rapide. Concurrent de Haiku. Bon pour les cas de RAG simple.

#### Meta / Open Source
- **Llama 4** : Sorti avril 2026. Modèle 400B paramètres. Excellent pour du self-hosting. Use case H'appi : clients qui veulent du on-premise (banques, hôpitaux).
- **Mistral Large 3** : Très bon sur le français. Alternative viable pour les clients sensibles aux données.

### 13.2 Frameworks & Infrastructure IA

#### LangChain / LangGraph
- **LangGraph 0.3** : Framework d'agents avec état persistant. Parfait pour les workflows multi-étapes. Intégration native avec Claude.
- **LangSmith** : Observabilité des LLM. Tracker tokens, latence, erreurs en prod. Gratuit jusqu'à 5K traces/mois.

#### Vector Databases
- **pgvector 0.8** : Maintenant stable pour la prod. Extension PostgreSQL → pas besoin d'une DB séparée. ← **Recommandé pour Happi Brain V2**
- **Pinecone** : Toujours la référence pour >1M vecteurs. Pricing: $0.096/heure pour pod p1.
- **Qdrant** : Open source, self-hostable. Très bonnes perfs. Alternative à Pinecone pour clients on-premise.

#### Orchestration
- **n8n** : Concurrent Make/Zapier, open source. Parfait pour les workflows IA automatisés. Self-hostable.
- **Temporal** : Pour les workflows long-running (>1 min). Utile pour les pipelines de traitement de documents.

### 13.3 Frontend & Mobile

#### Next.js
- **Next.js 16** : Turbopack par défaut (stable). Partial Prerendering (PPR) en beta. Server Actions matures. ← **Stack actuelle H'appi**
- **React 19** : Concurrent features stables. React Compiler (beta) : optimisation auto des re-renders.

#### React Native / Mobile
- **Expo SDK 54** : SDK actuel. EAS Build pour CI/CD mobile. ← **Stack actuelle H'appi**
- **Expo Router v4** : File-based routing, web support amélioré.

#### Animation
- **Framer Motion 12** : API unifiée web/native. `useAnimate` hook recommandé plutôt que `variants` pour la complexité. ← **Stack actuelle H'appi**
- **Motion for React Native** : Même API que Framer Motion mais pour RN. Encore en beta.

### 13.4 Tooling & DevOps

#### CI/CD
- **GitHub Actions** : Standard. Utiliser les runners Ubuntu 24.04 (plus rapide).
- **Railway** : Déploiement automatique depuis GitHub. Très stable pour FastAPI + PostgreSQL. ← **Stack actuelle H'appi**

#### Observabilité
- **Sentry** : Toujours la référence. Error tracking + performance monitoring. Plan gratuit suffisant pour les projets H'appi early-stage.
- **PostHog** : Analytics produit open source. Alternative à Amplitude. Self-hostable.

#### Sécurité
- **Vault (HashiCorp)** : Gestion des secrets en prod. Pour les clients Enterprise.
- **SOPS + Age** : Chiffrement des secrets dans le repo Git. Plus simple que Vault pour les petits projets.

### 13.5 Tendances à surveiller (Q3 2026)

1. **Agents IA autonomes** : Les agents multi-étapes (LangGraph, CrewAI) vont remplacer les chatbots simples. H'appi devrait proposer une offre "Agent" en plus du "Chatbot".

2. **Voice AI 2.0** : Vapi.ai a levé $20M. ElevenLabs sort une API Conversational AI. La latence voix tend vers 200ms. Le marché du secrétariat vocal va exploser en France.

3. **RAG → GraphRAG** : Microsoft GraphRAG indexe les relations entre documents (pas juste les chunks). Meilleur pour les bases de connaissance complexes (juridique, médical).

4. **MCP (Model Context Protocol)** : Anthropic a standardisé le protocole. De plus en plus d'outils publient des serveurs MCP. Happi Brain V2 devrait être un serveur MCP.

5. **AI Act EU** : Entré en vigueur juin 2026. Les chatbots doivent s'identifier comme IA. Impact direct sur nos chatbots. Pour H'appi : inclure systématiquement la mention "Ce service est géré par un assistant virtuel IA" dans chaque livraison + proposer un audit RGPD + AI Act comme service additionnel payant.
